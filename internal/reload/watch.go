...
func (reload *Reloader) WatchDirectories() {
	...
	for _, path := range reload.directories {
		directories, err := reload.recursiveWalk(path)
		if err != nil {
	...
	for {
		...
		case e := <-w.Events:
			if reload.ShouldIgnore != nil && reload.ShouldIgnore(e.Name) {
				continue
			}
			switch {
			...
			case e.Has(fsnotify.Rename), e.Has(fsnotify.Remove):
				...
				directories, _ := reload.recursiveWalk(e.Name)
...
func (reload *Reloader) recursiveWalk(path string) ([]string, error) {
	var res []string
	err := filepath.WalkDir(path, func(path string, d fs.DirEntry, err error) error {
		...
		if reload.ShouldIgnore != nil && reload.ShouldIgnore(path) {
