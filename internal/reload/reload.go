...
//
// Vendored from github.com/aarol/reload v1.2.0 (MIT, see LICENSE in this
// directory) with one addition: the ShouldIgnore hook, which lets callers
// exclude directories (e.g. node_modules, .git) from the recursive file
// watcher so large trees don't exhaust inotify watches.
//
...
type Reloader struct {
	...
	OnReload func()
	// ShouldIgnore, when non-nil, reports whether a path should be excluded
	// from watching. Directories it matches are not descended into, and
	// filesystem events under matching paths do not trigger a reload.
