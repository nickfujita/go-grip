...
func TestDirectoryListingIgnoresCacheValidators(t *testing.T) {
	...
	server := NewServer("localhost", 6419, false, false, false, NewParser(), nil, "auto", nil)
...
func TestCustomCSSIsServedAtStableRoute(t *testing.T) {
	...
	server := NewServer("localhost", 6419, false, false, false, NewParser(), []string{cssPath}, "auto", nil)
	handler := server.newHandler(http.Dir(tmpDir))
